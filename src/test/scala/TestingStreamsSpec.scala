...
class TestingStreamsSpec extends TestKit(ActorSystem("testing-akka-streams"))
                         with WordSpecLike
    ...
    "A simple stream" should {
        ...
        "integrate with a test-actor-based sink" in {
            val anotherSource = Source(1 to 5)
            val flow = Flow[Int].scan[Int](0)(_ + _) // 0, 1, 3, 6, 10, 15
            val streamUnderTest = anotherSource.via(flow)
        ...
        "integrate with Streams TestKit Sink" in {
            ...
            materializedTestValue
                .request(5) // This will ask the testSink to request some demand from the Source. In this case 5 numbers
                .expectNext(2, 4, 6, 8, 10) // tell which values are expected
                .expectComplete() // expects the termination of the stream
            // if fewer numbers are requested (like 4), the stream will never complete and will fail
        }
        
        "integrate with Streams TestKit Source" in {
            val sinkUnderTest = Sink.foreach[Int] {
                case 13 => throw new RuntimeException("PT")
                case _  => println
            }
            
            val testSource = TestSource.probe[Int]
            val materialized = testSource.toMat(sinkUnderTest)(Keep.both).run()
            // ^ it will return a tuple with the TestSource.probe result and the Future[Done] result
